    ...
    def run(self, n_turns):
        ...
        for turn in range(self.n_turns):
            ...
            for i in range(self.n_interactions-1):
                ...
                #Ex, Ey = get_field(particles.x, particles.y, sigma_x, sigma_y)
                #particles.px += coef * l * Ex
                #particles.py += coef * l * Ey

            self._track_section(start=f'spacecharge_{self.n_interactions-1}', 
                                stop=0)
