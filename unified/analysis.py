...
class Analysis:
    ...
    def plot_spectrum(self, field, i=0):
        if self.n_macro_particles==(1 or 'co'):
            ...
            if field=='x':
                field=self.x_scbsc[i]
            elif field=='y':
                field=self.y_scbsc[i]
            elif field=='px':
                field=self.px_scbsc[i]
            elif field=='py':
                field=self.py_scbsc[i]
        
        N = len(field)
        dt = 1 / self.n_interactions
        fft = np.fft.fft(field) 
        freqs = np.fft.fftfreq(N, d=dt)

        fig, ax = plt.subplots(figsize=(20, 8))
        ax.plot(freqs, abs(fft))
        ax.set_xlim(0, self.n_interactions/2)
        ax.set_xticks(np.arange(0, (self.n_interactions+1)/2, 2))
